...
# local imports
from lib.rule import make_all_ones, make_zeros, rule_vand, rule_vandnot, rule_vectompz, rule_mpztovec, count_ones
...
class Tree:
    ...
    def __init__(self, cache_tree, ndata, lamb, splitleaf=None, prior_metric=None):
        ...
        if leaves[0].num_captured == ndata:
            ...
            raise 'Error: Unsupported priority metric ' + str(prior_metric)
...
def bbound(x, y, lamb, prior_metric=None, MAXDEPTH=float('Inf'), MAX_NLEAVES=float('Inf'), niter=float('Inf'), logon=False,
    ...
    while queue and COUNT < niter and time.time() - tic < timelimit:
        ...
        leaf_split = tree.splitleaf # [1] bitarray selecting the split leaves
        removed_leaves = list(compress(leaves, leaf_split)) # select the leaves to split
    ...
    def predictor(features, labels=None):
        ...
        for leaf in leaves_c:
            ...
            for feature in leaf:
                ...
                cap = (features[:, idx] == feature_label) * cap
            caps.append(cap)
