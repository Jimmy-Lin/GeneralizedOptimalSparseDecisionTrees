...
class CacheTree:
    ...
    def __init__(self, lamb, leaves):
        self.leaves = leaves
        self.risk = sum([l.loss for l in leaves]) + lamb * max([len(l.rules) for l in leaves])  # len(leaves)
...
class Tree:
    ...
    def __init__(self, cache_tree, ndata, lamb, splitleaf=None, prior_metric=None):
        ...
        self.lb = sum([cache_tree.leaves[i].loss for i in range(l)
                       if splitleaf[i] == 0]) + lamb * max([len(l.rules) for l in leaves])#l
...
def bbound_penalize_depth(x, y, lamb, prior_metric=None, MAXDEPTH=float('Inf'), MAX_NLEAVES=float('Inf'), niter=float('Inf'), logon=False,
    ...
    while queue and COUNT < niter and time.time() - tic < timelimit:
        ...
        tree_depth = max([len(l.rules) for l in leaves])
        removed_leaves_depth = max([len(l.rules) for l in removed_leaves])

        new_tree_depth = max(removed_leaves_depth + 1, tree_depth)

        # prefix-specific upper bound on prefix length
        if lenbound and new_tree_depth >= min(tree_depth + math.floor((R_c - tree.lb) / lamb),
                                               nrule):
            # print("toolong===COUNT:", COUNT)
            continue

        # lookahead when the depth will increase
        lambbb = lamb if lookahead and tree_depth == removed_leaves_depth else 0
        if lb + b0 + lambbb >= R_c:
            continue
