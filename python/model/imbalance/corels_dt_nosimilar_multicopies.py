...
from core.rule import make_all_ones, make_zeros, rule_vand, rule_vandnot, rule_vectompz, rule_mpztovec, count_ones
...
def bbound(x, y, lamb, prior_metric=None, MAXDEPTH=float('Inf'), MAX_NLEAVES=float('Inf'), niter=float('Inf'), logon=False,
    ...
    leaf_cache[make_all_ones(ndata + 1)] = root_leaf
        ...
        for leaf_rules in product(*rules_for_leaf):
            ...
            for rule, removed_leaf in zip(leaf_rules, removed_leaves):
                ...
                for new_rule in (-rule, rule):
                    ...
                    # print("IndexError:", rule_index, len(x_mpz))
                    #print("x_mpz",x_mpz)
                    ...
                    if new_points_cap not in leaf_cache:
                        ...
                        leaf_cache[new_points_cap] = new_leaf
                        new_leaves.append(new_leaf)
                    else:
                        # new_leaf = leaf_cache[new_rules]
                        new_leaf = leaf_cache[new_points_cap]
                        new_leaves.append(new_leaf)
            ...
            # NEW_KEY
            sorted_new_tree_rules = tuple(sorted(leaf.points_cap for leaf in new_tree_leaves))
